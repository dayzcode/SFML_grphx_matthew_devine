...
#pragma region Create Graph points 
    ...
    {
        addPoint(sf::Color(15,250,90), 1, sf::Vector2u(1, 1), sf::Vector2f(0, 0)); // colour of points
        for (int j = 0; j < cords[j].size(); ++j) // for number of columns (2)
        {
            ...
            if (j == 1) points[i].setPosition(sf::Vector2f(points[i].getPosition().x + 100 + pointsGap * i, _win.getSize().y - (60 + 6 * float(stoi(cords[i][j]))))); 
        }
    }
#pragma endregion
